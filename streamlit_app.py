"""
Business Analyzer – Production Ready with PostgreSQL Support
- Uses SQLite for local development
- Uses PostgreSQL for production (Streamlit Cloud / Docker)
...
class Config:
    ...
    JWT_ALGORITHM = 'HS256'
    ADMIN_PASSWORD = os.environ.get('ADMIN_PASSWORD', 'Project@123')
...
# -----------------------------------------------------------------------------
# Database Manager with Dialect Detection
# -----------------------------------------------------------------------------
class DBManager:
    _engine = None
    _dialect = None
    ...
    def get_engine(cls):
        if cls._engine is None:
            ...
            print(f"DEBUG: Database engine created. Dialect: {cls._dialect}", file=sys.stderr)
        return cls._engine
    ...
    def init_db(cls):
        with cls.get_connection() as conn:
            if cls._dialect == 'postgresql':
                id_def = "SERIAL PRIMARY KEY"
                int_def = "SERIAL PRIMARY KEY"
            else:
                id_def = "INTEGER PRIMARY KEY AUTOINCREMENT"
                int_def = "INTEGER PRIMARY KEY AUTOINCREMENT"
    ...
    def insert_and_get_id(cls, query, params=None, conn=None):
        ...
        if cls._dialect == 'postgresql' and 'RETURNING' not in query.upper():
...
def logout():
    if st.session_state.current_login_id:
        if DBManager._dialect == 'postgresql':
            DBManager.execute(
                """
